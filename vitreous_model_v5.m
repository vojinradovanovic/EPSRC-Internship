...
forceTypes = [sin(sol.t*args.scalePeriod);
                ...
                -(1-exp(-4*sol.t)-1/(1+exp(2-2*sol.t)))*10^(-2);
                +(1-exp(-4*sol.t)-1/(1+exp(2-2*sol.t)))*10^(-3);
...
for i = 1:size(sol.indexFields, 1)
    ...
    if fieldVal ~= 0
        ...
        elseif field == "ILM" && ~args.onlyAngle
            ...
            % ILM with what it's connected to
            if hasBase
                field = "Base+ILM";
            elseif hasStalk
                field = "MCC+ILM";
        ...
        elseif field=="MCC" && ~args.onlyAngle
            if args.hasPrint
                fprintf(field+" stress: ");
            end
            sol.varSols(:, j+1) = getVarSol(sol.vars(fieldVal, 1), sol, const, args.hasPrint);
